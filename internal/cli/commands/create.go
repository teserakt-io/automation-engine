...
func NewCreateCommand(c2seClientFactory grpc.ClientFactory) Command {
	...
	cobraCmd.Flags().StringVarP(&createCmd.flags.Description, "description", "d", "", "short description of the rule")
	cobraCmd.Flags().StringVarP(&createCmd.flags.Action, "action", "a", "", "action to be performed when the rule will trigger")
